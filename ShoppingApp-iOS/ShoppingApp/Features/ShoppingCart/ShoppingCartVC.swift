...
class ShoppingCartVC: UIViewController {
    ...
    private func presentCardInputModal() {
        let alert = UIAlertController(title: "Kart Numarası", message: "Lütfen 14 haneli kart numaranızı girin", preferredStyle: .alert)
        alert.addTextField { textField in
            textField.placeholder = "Kart Numarası"
        ...
        alert.addAction(UIAlertAction(title: "Onayla", style: .default, handler: { [weak self] _ in
            guard let self = self,
                ...
                self?.showInvalidCardAlert()
                return
            }

            self.viewModel.cardNumber.value = input
            self.viewModel.confirmCheckout()
    ...
    private func showInvalidCardAlert() {
        let alert = UIAlertController(title: "Hatalı Kart", message: "Kart numarası 14 haneli ve sadece rakamlardan oluşmalıdır.", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Tamam", style: .default, handler: nil))
        present(alert, animated: true)
    }

    private func showCheckoutSuccessAlert() {
